...
macOS용 개인 정리 유틸리티. CleanMyMac 스타일로 캐시·대용량 파일·앱 잔여물을 안전하게 찾아 정리한다. 모든 삭제는 휴지통을 거치며(완전 삭제 제외), 다크 전용 테마와 7개 언어 UI를 지원한다.
...
| 정리 | **스페이스 렌즈** | 큰 파일을 용량순 막대 차트로 시각화 (전체 보기 / 오래된 대용량 파일) |
...
**스페이스 렌즈** — 큰 파일을 용량순 막대 차트로 시각화

![스페이스 렌즈](docs/screenshots/space-lens.png)

**앱 제거** — 앱과 관련 잔여 파일까지 함께 제거, 용량·미사용 앱 통계 제공
...
Sources/Abysso/       # 앱 소스 (SwiftUI 뷰 + 모델, 31개 파일)
  ...
  LargeFilesView.swift# 스페이스 렌즈 (병렬 스캔)

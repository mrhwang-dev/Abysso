...
final class UninstallModel: ObservableObject {
    ...
    nonisolated static func findApps() -> [InstalledApp] {
    ...
    func findRelated(for app: InstalledApp) {
        ...
        findingRelated = true
        Task.detached(priority: .userInitiated) {
            let result = Self.relatedFiles(for: app)
            await MainActor.run {
    ...
    nonisolated static func relatedFiles(for app: InstalledApp) -> [RelatedFile] {
        let fm = FileManager.default
        ...
        for (path, kind) in candidates {
            ...
            found.append(RelatedFile(url: url, size: DiskUtil.itemSize(url), kind: kind))
        }
    ...
    func uninstall() {
        ...
        message = appFailed
            ? "앱 본체를 옮기지 못했습니다 (실행 중이거나 권한 부족). 관련 파일 \(Format.bytes(freed)) 는 휴지통으로 옮겼습니다."
            : "\(app.name) 및 관련 파일 \(Format.bytes(freed)) 를 휴지통으로 옮겼습니다."
        selectedAppID = nil
        relatedFiles = []
        scanApps()
    }
    ...
    func reset() {
        ...
        message = "\(app.name) 을(를) 재설정했습니다 — 관련 파일 \(Format.bytes(freed)) 를 휴지통으로 옮겼습니다. 앱은 그대로 유지됩니다."
        findRelated(for: app)
    }
    ...
    func scanOrphans() {
        ...
        Task.detached(priority: .userInitiated) {
            let result = Self.findOrphans(installedIDs: installedIDs)
            await MainActor.run {
                withAnimation {
                    self.orphans = result
    ...
    nonisolated static func findOrphans(installedIDs: Set<String>) -> [OrphanFile] {
        let fm = FileManager.default
        let home = fm.homeDirectoryForCurrentUser
        ...
        for dir in dirs {
            ...
            for url in items {
                ...
                if size > 100_000 {
                    found.append(OrphanFile(url: url, size: size, bundleID: name))
                }
            }
        }
        found.sort { $0.size > $1.size }
    ...
    func trashSelectedOrphans() {
        ...
        message = "잔여 파일 \(Format.bytes(freed)) 를 휴지통으로 옮겼습니다."
                            ...
                            VStack(alignment: .leading, spacing: 1) {
                                ...
                                if app.isUnused {
                                    Text(app.lastUsed.map {
                                        "마지막 사용: " + $0.formatted(.relative(presentation: .named))
                                    } ?? "사용 기록 없음")
                                        .font(.caption2)
        ...
        if let message = model.message {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text(app.bundleID ?? "번들 ID 없음")
                            ...
                            .foregroundStyle(.secondary)
                        HStack(spacing: 8) {
                            Text(app.size > 0 ? "앱 \(Format.bytes(app.size))" : Format.unknownSize)
                ...
                if model.findingRelated {
                    Spacer()
                    HStack { Spacer(); ProgressView("관련 파일을 찾는 중…"); Spacer() }
                    Spacer()
                } else {
    ...
    private var summaryDashboard: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 12) {
                    statTile(value: "\(model.apps.count)개", label: "설치된 앱", color: Theme.blue)
                    statTile(value: Format.bytes(model.totalSize), label: "총 용량", color: Theme.teal)
                    statTile(
                        value: "\(model.unusedCount)개",
        ...
        VStack(spacing: 0) {
            if model.scanningOrphans {
                ...
                Image(systemName: "checkmark.seal.fill")
                    .font(.system(size: 40))
                    .foregroundStyle(Theme.green)
                Text("잔여 파일이 없습니다 — 깨끗합니다!")
                    .font(.headline)
                    .padding(.top, 8)
                if let message = model.message {
                    Text(message).font(.callout).foregroundStyle(.secondary).padding(.top, 4)
                }
                Spacer()
            } else {
                HStack {

...
final class OptimizationModel: ObservableObject {
    ...
    func quit(pids: [Int32], name: String, force: Bool) {
        var allOk = true
        for pid in pids {
            if !ProcessMonitor.quit(pid: pid, force: force) { allOk = false }
        }
        message = allOk
            ? String(format: NSLocalizedString(force ? "%@ 을(를) 강제 종료했습니다" : "%@ 을(를) 종료했습니다", comment: ""), name)
            : String(format: NSLocalizedString("%@ 종료에 실패했습니다", comment: ""), name)
...
struct OptimizationView: View {
    ...
    private func agentCard(title: LocalizedStringKey, subtitle: LocalizedStringKey, items: [LaunchAgentInfo]) -> some View {
                ...
                ForEach(procs) { proc in
                    HStack(spacing: 10) {
                        ...
                        Text(valueText(proc))
                            ...
                            .monospacedDigit()
                        Button {
                            model.quit(pids: proc.pids, name: proc.name, force: false)
                        } label: {
    ...
    private var runningSection: some View {
        ...
        return VStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Divider().opacity(0.3)
                if model.guiApps.isEmpty {
                    EmptyStateView(
                        icon: "macwindow",
                        title: "실행 중인 앱이 없습니다",
                        message: "현재 백그라운드를 제외하고 열려 있는 일반 사용자 앱이 없습니다.",
                        tint: Theme.teal
                    )
                } else {
    ...
    private func appRow(_ app: NSRunningApplication, unresponsive: Bool) -> some View {
        HStack(spacing: 10) {
            ...
            Text(LocalizedStringKey(app.localizedName ?? "알 수 없는 앱"))
                .font(.system(size: 13))
            if unresponsive {
                Text("응답 없음")
                    .font(.caption2.weight(.bold))
            ...
            Spacer()
            Button {
                model.quit(pids: [app.processIdentifier],

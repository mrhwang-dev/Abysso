...
final class ExtensionsModel: ObservableObject {
    ...
    func setEnabled(_ item: ExtensionItem, _ enable: Bool) {
        ...
        busy.insert(item.id)
        Task.detached(priority: .userInitiated) {
            var performCommands = true
            #if DEBUG
            performCommands = DevMode.shouldRunCommand(
                "pluginkit -e \(enable ? "use" : "ignore") -i \(item.identifier)"
            )
            #endif
            if performCommands {
    ...
    func trash(_ item: ExtensionItem) {
        ...
        if DiskUtil.trash(url) {
            ...
            message = String(format: NSLocalizedString("%@ 을(를) 휴지통으로 옮겼습니다", comment: ""), item.name)
        } else {
            message = String(format: NSLocalizedString("%@ 삭제 실패 — /Library 항목은 관리자 권한이 필요할 수 있습니다", comment: ""), item.name)
        }
    }
}
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(ExtKind.allCases, id: \.self) { kind in
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack {
                            Label(LocalizedStringKey(kind.rawValue), systemImage: kind.icon)
                                .font(.headline)

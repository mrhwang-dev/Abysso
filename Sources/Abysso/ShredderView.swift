...
final class ShredderModel: ObservableObject {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                ...
                self.finishedMessage = failed == 0
                    ? String(format: NSLocalizedString("%lld개 항목(%@)을 복구 불가능하게 파쇄했습니다.", comment: ""), targets.count, Format.bytes(freed))
                    : String(format: NSLocalizedString("%lld개 파쇄 완료, %lld개 실패 (권한 또는 사용 중)", comment: ""), targets.count - failed, failed)
    ...
    nonisolated static func shred(_ url: URL) -> Bool {
        #if DEBUG
        // 덮어쓰기+삭제는 되돌릴 수 없으므로 테스트 모드에서는 특히 엄격히 차단한다.
        if DevMode.dryRunEnabled {
            DevMode.log("DRY-RUN", "🗑️", "파쇄(덮어쓰기+삭제) 예정: \(url.path)")
            return true
        }
        if DevMode.sandboxScanEnabled && !DevMode.isInsideSandbox(url) {
            DevMode.log("SANDBOX", "🛡️", "샌드박스 외부 보호 — 파쇄 차단: \(url.path)")
            return true
        }
        #endif
...
struct ShredderView: View {
    ...
    private var dropZone: some View {
        EmptyStatePane(
            ...
            title: "민감한 파일을 흔적 없이 지우세요",
            message: "파일이나 폴더를 여기로 드래그하거나 아래 버튼으로 선택하세요.\n랜덤 데이터로 여러 번 덮어쓴 뒤 삭제하므로 복구 도구로도 되살릴 수 없습니다.",
            glow: Theme.red,
            footnote: "참고: 최신 Mac(SSD) 환경에서는 시스템 최적화 기능으로 인해 완벽한 파쇄가 제한될 수 있습니다."
        ) {
            ProminentScanButton(title: "파일 선택", systemImage: "plus.circle") {
                importing = true
            }
        }
        // 드롭존 점선 테두리 (드래그 중이면 강조)
        .overlay {
        ...
        VStack(spacing: 0) {
            if model.shredding {
                ProgressView(value: model.progress)
                    ...
                    .padding(.horizontal, 24)
                    .padding(.top, 8)
                // 파쇄가 길어지면 경과 시간과 이유를 알려준다
                ScanDelayNotice(reason: "파일 용량이 커서 덮어쓰기가 지연되고 있습니다")
                    .padding(.bottom, 8)
            }
            List {

...
struct RunningProc: Identifiable {
    ...
    var count: Int { pids.isEmpty ? 1 : pids.count }
...
enum ProcessMonitor {
    ...
    static func topProcesses(limit: Int = 12) -> [RunningProc] {
        ...
        var grouped: [String: RunningProc] = [:]
        for line in result.output.split(separator: "\n") {
            ...
            if var existing = grouped[name] {
                existing.cpu += cpu
                existing.memBytes += rssKB * 1024
                existing.pids.append(pid)
                grouped[name] = existing
            } else {
                grouped[name] = RunningProc(
                    pid: pid, pids: [pid], name: name, cpu: cpu,
                    memBytes: rssKB * 1024, bundleID: bundleByPID[pid]
                )
            }
        }
        procs = Array(grouped.values)
        return procs
    }
    ...
    static func quit(pid: Int32, force: Bool) -> Bool {
        #if DEBUG
        if !DevMode.shouldRunCommand("프로세스 종료 (pid \(pid), \(force ? "SIGKILL" : "SIGTERM"))") {
            return true
        }
        #endif

    ...
    nonisolated static func findPrivacyData() -> [PrivacyCategory] {
        ...
        for (browser, basePath) in chromiumBrowsers {
            ...
            if !items.isEmpty {
                result.append(PrivacyCategory(
                    title: browser,
                    subtitle: String(format: NSLocalizedString("정리 전 %@를 종료하세요", comment: ""), browser),
        ...
        if let profiles = try? fm.contentsOfDirectory(at: firefoxProfiles, includingPropertiesForKeys: nil) {
            ...
            if !items.isEmpty {
                result.append(PrivacyCategory(
                    title: "Firefox",
                    subtitle: String(format: NSLocalizedString("정리 전 %@를 종료하세요", comment: ""), "Firefox"),
...
// MARK: - 뷰
//
// 별도 탭이 아니라 '스마트 정리' 탭의 '개인정보 보호' 구획으로 임베드된다.
// (페이지 헤더는 CacheView가 그리므로 여기에는 없다)
...
struct PrivacyView: View {
    ...
    private var emptyStartView: some View {
        EmptyStatePane(
            ...
            iconStyle: AnyShapeStyle(Theme.blue.gradient),
            title: "나의 흔적을 깨끗하게",
            message: "브라우저에 쌓인 방문 기록·쿠키·다운로드 기록과 macOS의\n최근 사용 항목을 찾아 한 번에 지울 수 있습니다.",
            glow: Theme.blue
        ) {

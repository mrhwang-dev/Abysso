...
final class MaintenanceModel: ObservableObject {
    ...
    static let tasks: [MaintenanceTask] = [
        ...
        MaintenanceTask(
            ...
            subtitle: "검색 결과가 이상하거나 누락될 때 — 색인을 다시 만듭니다 (완료까지 수십 분간 Mac이 다소 느려질 수 있음)",
    ...
    func run() {
        ...
        Task {
            ...
            withAnimation {
                ...
                running = false
                if ok {
                    message = "\(targets.count)개 작업 완료 (\(String(format: "%.1f", elapsed))초). " +
                        (selected.contains("spotlight") ? "Spotlight 재색인은 백그라운드에서 계속 진행됩니다." : "")
                    selected = []
                } else {
                    message = "실행이 취소되었거나 실패했습니다 — 관리자 암호가 필요합니다."
                }
            }
            // 잠시 후 상태 아이콘 초기화
...
struct MaintenanceView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            PageHeader(
                title: "유지보수",
                subtitle: "Mac이 느려지거나 이상할 때 실행하는 시스템 최적화 작업 모음",
